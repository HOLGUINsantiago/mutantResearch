package com.stage.neuroPsi.service.alignement;
...
public abstract class SequenceAlignment extends DynamicProgramming {
   ...
   public int getAlignmentScore(int pos) {
      if (alignments == null) {
         getAlignment();
      }
   ...
   public boolean isGoodAlignmentScore(double min) {
      if (alignments == null) {
         getAlignment();
      }
      ...
      for (int i = 0; i < alignments[0].length(); i++) {
         ...
         if (((double) ((score + (alignments[0].length() - i) * match)) / sequence1.length()) > min) {
            return false;
         }
      }

      return (double) score / sequence1.length() > min;
   }
